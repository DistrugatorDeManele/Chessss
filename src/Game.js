...
export default class Game extends React.Component {
  constructor(props) {
    ...
    var ValidMoves = function() {
      this.numOfValidMoves = function() {
        var result = 0;
        for (var i in this)
          if (typeof this[i] === 'object') result += this[i].length;
        return result;
      };
      setValidMoves(this);
      ...
      function setValidMoves(vm) {
        ...
        if (Board['bar' + Board.onMove.capitalizeFirstLetter()][0] > 0) {
          ...
          for (var i = 0; i < Board.moves.length; i++) {
            if (
              isValid(
                'bar' + Board.onMove.capitalizeFirstLetter(),
                Board.moves[i]
              ) &&
              vm['bar' + Board.onMove.capitalizeFirstLetter()].indexOf(
                Board.moves[i]
              ) == -1
            )
              vm['bar' + Board.onMove.capitalizeFirstLetter()].push(
                Board.moves[i]
              );
          }
        } else {
          // if the player on move has not any men on the bar
          for (var i = 1; i <= 24; i++) {
            if (Board['p' + i][1] != (Board.onMove == 'black' ? 0 : 1))
              continue;
            vm['p' + i] = [];
            for (var j = 0; j < Board.moves.length; j++) {
              if (
                isValid('p' + i, Board.moves[j]) &&
                vm['p' + i].indexOf(Board.moves[j]) == -1
              )
                vm['p' + i].push(Board.moves[j]);
            }
          }
        }
      ...
      function isValid(start, number) {
        ...
        if (target.charAt(0) == 'p') {
          ...
          if (startPt == (Board.onMove == 'black' ? number : 25 - number))
            return true;
  ...
  drawBoard(brd) {
    ...
    function drawPoint(j, point) {
      ...
      for (var i = 0; i < numCheckers; i++) {
        ...
        drawChecker(
          ...
          centery,
          color,
          j == brd.mouseOverPoint &&
            i == numCheckers - 1 &&
            brd.status == 'move' &&
            point[1] == (brd.onMove == 'black' ? 0 : 1)
        );
      }
      if (point[0] > 5) {
        //draw number to the last checker if they are more then 5
        ctx.save();
        ctx.fillStyle =
          color == blackCheckerColor ? whiteCheckerColor : blackCheckerColor;
    ...
    function drawChecker(x, y, col, hl) {
      ...
      ctx.beginPath();
      if (hl) {
        //highlight of the active checker by elevating it
        r *= 1.05;
        addShadow();
      }
    ...
    function drawRest() {
      ...
      function drawDices() {
        ...
        var diceColor =
          brd.onMove == 'black' ? blackDiceBodyColor : whiteDiceBodyColor;
        var dotColor =
          brd.onMove == 'black' ? blackDiceDotColor : whiteDiceDotColor;
        drawDice(
          ctx,
          x,
          y,
        ...
        x =
          brd.onMove == 'black' ? x + h * p * 4 + w * p : x - h * p * 4 - w * p;
        drawDice(
          ctx,
          x,
          y,
          h * p * 4,
          Dices.diceValueRight,
          diceColor,
          dotColor
        );
        Dices.onScreen = true;
      ...
      function drawAcceptButton() {
        ...
        takeButton.x =
          brd.onMove == 'black'
            ? w * p * 36 - takeButton.width / 2 - (w * p) / 2
            : w * p * 14 + takeButton.width / 2 + (w * p) / 2;
        passButton.x =
          brd.onMove == 'black'
            ? w * p * 36 + takeButton.width / 2 + (w * p) / 2
            : w * p * 14 - takeButton.width / 2 - (w * p) / 2;
  ...
  handleMousemove(evt) {
    ...
    Board.mouseOverPoint =
      mousePos.point != undefined && mousePos.point.charAt(0) == 'p'
        ? parseInt(mousePos.point.substr(1))
        : undefined;
    drawBoard(Board);
    printMousePos();
  }
  handleMouseclick(evt) {
    ...
    if (Board.status == 'start' && mousePos.point == 'startButton') {
      ...
      document.getElementById('statusInfo').innerHTML =
        Board.onMove.capitalizeFirstLetter() + ' starts the game.';
    ...
    if (Board.status == 'roll') {
      if (mousePos.point == 'rollButton') {
        ...
        Board.moves.sort();
        document.getElementById('statusInfo').innerHTML =
          'Rolled numbers: ' + Dices.diceValueLeft + ' ' + Dices.diceValueRight;
        Board.status = 'move';
        ...
        rollButton.onScreen = false;
        document.getElementById('statusInfo').innerHTML =
          Board.onMove.capitalizeFirstLetter() + ' has doubled. Do you accept?';
        Board.onMove = Board.onMove == 'black' ? 'white' : 'black';
    ...
    if (Board.status == 'move') {
      ...
      if (mousePos.point.charAt(0) == 'p' || mousePos.point.charAt(0) == 'b') {
        if (Board[mousePos.point][1] != (Board.onMove == 'black' ? 0 : 1)) {
          document.getElementById('statusInfo').innerHTML =
            'This is not your checker!';
          return;
        }
        if (Board.moves.length == 0) {
          document.getElementById('statusInfo').innerHTML =
            'You have moved already. Pick up the dice!';
          return;
        }
        start = mousePos.point;
        if (!valids.hasOwnProperty(start) || valids[start].length == 0) {
          document.getElementById('statusInfo').innerHTML =
            "There aren't any valid moves from here.";
          return;
        }
        stpt =
          start.charAt(0) == 'p'
            ...
            : 0;
        var currentMove =
          valids[start].length > 1 ? Dices.diceValueLeft : valids[start][0];
        tgpt =
          Board.onMove == 'black' ? stpt - currentMove : stpt + currentMove;
        ...
        if (valids.numOfValidMoves() == 0) {
          ...
          if (Board['off' + Board.onMove.capitalizeFirstLetter()][0] == 15) {
            document.getElementById('statusInfo').innerHTML =
              Board.onMove.capitalizeFirstLetter() +
              ' won the game. Press start for a new one.';
            Dices.onScreen = false;
            Board.status = 'start';
            drawBoard(Board);
            return;
    ...
    if (Board.status == 'accept') {
      if (mousePos.point == 'takeButton') {
        ...
        Board.status = 'start';
        document.getElementById('statusInfo').innerHTML =
          winner + ' won the game. Press start for a new one.';
        drawBoard(Board);
      }
    }
  }
  move(start, target) {
    ...
    if (target.charAt(0) == 'o')
      Board['off' + Board.onMove.capitalizeFirstLetter()][0]++;
  ...
  getMousePos(canvas, evt) {
    ...
    */
    ...
    if (pt != 0) result.point = 'p' + pt;
      ...
      result.xp > 23.5 &&
      result.xp < 26.5 &&
      result.y < h * p * 18 + w * p * 1.5 &&
      result.y > h * p * 18 - w * p * 3 * (Board.barBlack[0] - 0.5)
    )
      ...
      result.xp > 23.5 &&
      result.xp < 26.5 &&
      result.y > h * p * 32 - w * p * 1.5 &&
      result.y < h * p * 32 + w * p * 3 * (Board.barWhite[0] - 0.5)
    )
      result.point = 'barWhite';
    else if (
      rollButton.onScreen &&
      result.x > rollButton.x - rollButton.width / 2 &&
      result.x < rollButton.x + rollButton.width / 2 &&
      result.y > rollButton.y - rollButton.height / 2 &&
      result.y < rollButton.y + rollButton.height / 2
    )
      result.point = 'rollButton';
    else if (
      startButton.onScreen &&
      result.xp > 21 &&
      result.xp < 29 &&
      result.yp > 23 &&
      result.yp < 27
      ...
      takeButton.onScreen &&
      result.x > takeButton.x - takeButton.width / 2 &&
      result.x < takeButton.x + takeButton.width / 2 &&
      result.y > takeButton.y - takeButton.height / 2 &&
      result.y < takeButton.y + takeButton.height / 2
    )
      result.point = 'takeButton';
    else if (
      passButton.onScreen &&
      result.x > passButton.x - passButton.width / 2 &&
      result.x < passButton.x + passButton.width / 2 &&
      result.y > passButton.y - passButton.height / 2 &&
      result.y < passButton.y + passButton.height / 2
    )
      result.point = 'passButton';
    else if (
      Cube.onScreen &&
      result.x > Cube.x - Cube.size / 2 &&
      result.x < Cube.x + Cube.size / 2 &&
      result.y > Cube.y - Cube.size / 2 &&
      result.y < Cube.y + Cube.size / 2
    )
      result.point = 'Cube';
    else if (Dices.onScreen) {
      if (
        Dices.position == 'right' &&
        result.xp > 32.5 &&
        result.xp < 39.5 &&
        result.yp > 23 &&
        result.yp < 27
